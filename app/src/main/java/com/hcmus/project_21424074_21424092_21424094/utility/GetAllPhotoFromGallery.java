package com.hcmus.project_21424074_21424092_21424094.utility;
...
public class GetAllPhotoFromGallery {
    ...
    public static void removeImageFromAllImages(String path) {  // remove deleted photo from "database"
        Log.d("HCMUS GALLERY","GetAllPhotosFromGallery -> Trying to remove "+path);
        for(int i=0;i<allImages.size();i++) {
            if(allImages.get(i).getPath().equals(path)) {
                Log.d("HCMUS GALLERY","GetAllPhotosFromGallery -> Image removed from allImages. Breaking");
                allImages.remove(i);
                break;
            }
        }
